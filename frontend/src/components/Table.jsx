...
function Table(props) {
    ...
    const totalItems = props.csvData.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentData = props.csvData.slice(startIndex, endIndex);
    ...
    if (!props.csvData || props.csvData.length === 0) {
        return <div className={`flex items-center justify-center rounded shadow-xl p-8 min-h-[300px] ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-400 text-black'}`}>No data to display. Please upload a CSV file.</div>;
    }
